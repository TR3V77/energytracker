"""PROJ-114: Verify month-over-month energy trends."""
...
def test_trends_first_month_pct_change_is_null(app, client):
    ...
    first = next(
        t for t in trends if t["period"] == "2026-01"
    )
...
def test_trends_pct_change_calculation(app, client):
    ...
    feb = next(
        t for t in trends if t["period"] == "2026-02"
    )

    expected_pct = round(
        ((feb["kwh"] - jan["kwh"]) / jan["kwh"]) * 100, 2
    )
...
def test_trends_filter_by_neighborhood(app, client):
    ...
    resp = client.get(
        "/api/analytics/trends?neighborhood_id=2"
    )
    ...
    assert feb["pct_change"] == 25.0

    mar = next(
        t for t in trends if t["period"] == "2026-03"
    )
    assert mar["kwh"] == 180.0
    assert mar["pct_change"] == -28.0


def test_trends_empty_when_no_data_for_neighborhood(client):
    resp = client.get(
        "/api/analytics/trends?neighborhood_id=999"
    )
    trends = resp.get_json()["trends"]
    assert trends == []
